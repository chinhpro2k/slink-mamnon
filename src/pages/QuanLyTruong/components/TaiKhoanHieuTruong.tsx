...
const TaiKhoanHieuTruong = (props: {
  ...
  React.useEffect(() => {
    ...
    getRoles();
    getLop();
    if (vaiTro === 'Admin' || vaiTro === 'SuperAdmin' || vaiTro === 'ChuTruong') {
      getTaiKhoanModel();
    }
  ...
  const handleDel = async (record: ITaiKhoanHieuTruong.Record) => {
    if (vaiTro === 'SuperAdmin' || vaiTro === 'Admin') {
      const res = await delVaiTroHieuTuong({ id: record?._id, truongId: props?.idTruong ?? '' });
      if (res?.status === 200) {
  ...
  const onFinish = async (values: any) => {
    const res = await addHieuTruong(values.id, props.newRecord._id, values.roles);
    if (res?.status === 200) {
    ...
    <>
      ...
      >
        { !props.disable && checkAllow('ADD_TK_HIEU_TRUONG') && (
          <Popconfirm
            title="Bạn đã có tài khoản hiệu trưởng trước đó chưa?"
            disabled={disableAdd()}
            onConfirm={() => {
              setEdit(false);
              setVisibleDrawer(true);
      ...
      <Modal
        title="Thêm tài khoản hiệu trưởng"
