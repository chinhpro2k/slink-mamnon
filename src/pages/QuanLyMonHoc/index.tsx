  ...
  const renderLast = (record: IMonHoc.Record) => {
    return (
      <React.Fragment>
        ...
        >
          <Button type="default" shape="circle" title="Xóa" disabled={!checkAllow('DEL_MON_HOC')}>
            <DeleteOutlined />
          </Button>
        </Popconfirm>
      </React.Fragment>
  ...
  return (
    <>
      <TableBase
        ...
        modelName="monhoc"
        title="Quản lý lĩnh vực phát triển chung"
        scroll={{ x: 1000, y: 400 }}
      >
          ...
          try {
            if (edit && saoChep) {
              ...
              if (res?.data?.statusCode === 201) {
                message.success('Sao chép lĩnh vực phát triển thành công');
                getMonHocCloneModel();
                setCondition({ donViId, chuongTrinhDaoTaoId: phuongPhapId });
                return true;
              }
            ...
            if (response?.data?.errorCode === 'NOT_ALLOW') {
              message.error('Bạn không được phép sao chép lĩnh vực phát triển ở đơn vị này!');
              return false;
            }
          }
          try {
            ...
            const { response }: any = error;
            if (response?.data?.errorCode === 'NOT_ALLOW') {
              message.error('Bạn không được phép cập nhật lĩnh vực phát triển ở đơn vị này!');
              return false;
            }
          }
          try {
            ...
            const { response }: any = error;
            if (response?.data?.errorCode === 'NOT_ALLOW') {
              message.error('Bạn không được phép thêm lĩnh vực phát triển ở đơn vị này!');
              return false;
            }
          }
          return false;
        ...
        <Form labelAlign="left" {...formItemLayout}>
          ...
          <Form.Item label="Phương pháp giáo dục" style={{ marginBottom: 0 }} required>
            <Select
              disabled
              value={
                `${newRecord?.chuongTrinhDaoTao?.ten ?? 'Không có'} ${
                  newRecord?.donVi?.tenDonVi
                    ? `- Thuộc ${newRecord?.donVi?.tenDonVi}`
                    : `- Thuộc trường chung`
                }` ?? 'Không có'
              }
            />
